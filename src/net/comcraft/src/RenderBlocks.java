...
public final class RenderBlocks {
    ...
    PolygonMode polygonModeCullingOff;
    PolygonMode polygonModeNormal;
    Material material;
    ...
    public Appearance appearanceListGet(int i)
    {
        if (appearanceList != null && appearanceList.length > i)
        {
            return appearanceList[i];
        }
        
        Appearance result;
        result = new Appearance();
        result.setTexture(0, cc.textureProvider.getTerrainTexture(i));
        result.setMaterial(material);
        result.setCompositingMode(compositingMode);
        ...
        if (cc.settings.fog) {
            ...
            if (cullingOffTextures.contains(new Integer(i))) {
                result.setPolygonMode(polygonModeCullingOff);
            } else {
                result.setPolygonMode(polygonModeNormal);
            }
            return result;
    }
...
//        material.setColor(Material.AMBIENT, 0x00FF0000);
        ...
        polygonModeNormal = new PolygonMode();
        ...
        polygonModeCullingOff = new PolygonMode();
        ...
        appearanceList = new Appearance[512];//512
    ...
    private void renderFenceFace(Block block, int x, int y, int z, int side, Transform transform) {
        ...
        if (blockVertexBuffer != null) {
            g3D.render(blockVertexBuffer[side % 6], block.getBlockIndexBuffer(), appearanceListGet(block.getBlockTexture(cc.world, x, y, z, side)), transform1);
        }
    }
    ...
    private void renderTransformedFace(Block block, int x, int y, int z, int side, Transform transform) {
        ...
        if (blockVertexBuffer != null) {
            g3D.render(blockVertexBuffer[side], block.getBlockIndexBuffer(), appearanceListGet(block.getBlockTexture(cc.world, x, y, z, side)), transform1);
        }
    }
    ...
    private void renderStandardFace(Block block, int x, int y, int z, int side, Transform transform) {
        ...
        if (blockVertexBuffer != null) {
            g3D.render(blockVertexBuffer[side], block.getBlockIndexBuffer(), appearanceListGet(block.getBlockTexture(cc.world, x, y, z, side)), transform);
        }
    }
    ...
    private void renderPiecedFace(Block block, int side, ChunkPiece chunkPiece, Transform transform) {
        if (chunkPiece.needsRender(cc.render.currentTick, side)) {
            g3D.render(chunkPiece.getVertexBuffer(cc.world, side), block.getBlockIndexBuffer(), appearanceListGet(block.getBlockTexture(cc.world, -1, -1, -1, side)), transform);
        }
    }
}
